...
def Windows_Run_Batch(fqbn, com, core, app):
	sys.stdout.write("Downloading core...\n")

	command_install_core = "arduino-cli core install " + core
	command_compile = "arduino-cli compile --fqbn " + fqbn + " arduino_files"
	...
	command_upload = "arduino-cli upload -p " + com + " --fqbn " + fqbn + " arduino_files"
	all_lines = []
...
def Linux_Run_Shell(fqbn, com, core, app):
	print(fqbn, com, core)
	all_line = []
	user = Linux_Darwin_Get_User()
	command = 'export PATH="/home/' + user + '/arduino-cli:$PATH";sudo chmod a+rw ' + com + ';arduino-cli core install '+core+'; arduino-cli compile --fqbn ' + fqbn + ' arduino_files; ' + 'arduino-cli upload -p ' + com + ' --fqbn ' + fqbn + ' arduino_files'
...
def Darwin_Run_Shell(fqbn, com, core, app):
	user = Linux_Darwin_Get_User()
	all_lines = []
	#command_board_list = 'export PATH="/Users/' + user + '/arduino-cli:$PATH"; arduino-cli core install ' + core + ' ;arduino-cli compile --fqbn ' + fqbn + ' arduino_files; ' + 'arduino-cli upload -p ' + com + ' --fqbn ' + fqbn + ' arduino_files'
	path_change = 'export PATH="/Users/' + user + '/arduino-cli:$PATH"'
	core_install = "arduino-cli core install " + core + " "
	compile = 'arduino-cli compile --fqbn ' + fqbn + ' arduino_files'
	upload = 'arduino-cli upload -p ' + com + ' --fqbn ' + fqbn + ' arduino_files'
...
def Start_Process(cli_is_installed, platform_name, app):
	...
	if(platform_name == "Windows"):
		Windows_Run_Batch(fqbn, com, core, app)

	elif(platform_name == "Linux"):
		Linux_Run_Shell(fqbn, com, core, app)

	elif(platform_name == "Darwin"):
		Darwin_Run_Shell(fqbn, com, core, app)

	else:
		print("OS not supported")

def Move_h(source_path, platform_name, curr_path, app):
	if(platform_name == "Windows"):
		h_name = "model.h"
		source_path = source_path.replace("/", "\\")
		destination_path = os.path.join(curr_path, "arduino_files", h_name)
		...
		dummy = os.path.join(curr_path, "arduino_files", "dummy.ino")
		original_ino = os.path.join(curr_path, "arduino_files", "arduino_files.ino")
...
def send_to_arduino(app):
	try:
		...
		if(h_path and h_path.split(".")[-1] == "h"): # this will run if there is any path available, maybe check for the extension?
			Move_h(h_path, platform_name, wd, app)
		else:
		...
		if(platform_name == "Windows"):
			...
			raise Exception("Code this!!")

		Start_Process(cli_is_installed, platform_name, app)
	except:
		return