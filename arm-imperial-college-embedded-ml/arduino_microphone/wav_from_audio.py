...
# Serial Port, check which port your Arduino is in (perhaps not COM3 for you)
COM_PORT = "COM3"
...
def record(labels):
    NUM_RECORDINGS = input("How many recordings per class/label?\n")
    while(not NUM_RECORDINGS.isnumeric()):
        NUM_RECORDINGS = input("Please enter a number!\n")

    for label in labels:
        for i in range(int(NUM_RECORDINGS)):
            ...
            for x in range(1, 4):
                if(x < 3):
                    print("Recording {} for label {} starts in {} seconds...".format(i+1, label, 3-x))
                    sleep(1)
                else:
                    print("Speak Now\n")
                    sleep(0.15)
            ...
            # make .wav file
            with wave.open("Generated_dataset/" + label + "/" + label + "_audio_file" + str(i) + ".wav", "wb") as out_f:
...
def get_labels():
    labels = input("Enter labels separated by commas like: 'A, B, C':\n").strip().split(",")
    ...
    for label in labels_final:
        if(not os.path.exists("Generated_dataset/" + label)):
            os.makedirs("Generated_dataset/" + label)
